...
void eFPartitionWalkByAddr(unsigned long efpHandler){
	...
	*/
    ...
    unsigned long current = efp->firstFree;
    while (current != 0) {
        EEB *eeb = (EEB *)current;
        showEEB(eeb);
        current = eeb->next_start;
    }
}
...
unsigned long eFPartitionInit(unsigned long start, unsigned long perSize, unsigned long n){
	...
	*/
    ...
    for (unsigned long i = 0; i < n; i++) {
        EEB *eeb = (EEB *)current;
        if (i == n - 1) {
            eeb->next_start = 0; // 最后一个块
        } else {
            eeb->next_start = current + blockSize;
        }
        current += blockSize;
    }
...
unsigned long eFPartitionAlloc(unsigned long EFPHandler){
	...
	*/
	...
	eFPartition *efp = (eFPartition *)EFPHandler;
    unsigned long freeBlock = efp->firstFree;
